...
| | 小红书 | B 站 |
|---|---|---|
| **User-Agent** | 自报身份，不伪装 | Chrome 151（仅 B 站；B 站拒绝非浏览器 UA，无法匿名解析） |
...
- 代码位置：`scripts/lib/bilibili-resolver.mjs:49-78`（指纹生成与持久化）
...
- 图片只从 `.xhscdn.com` / `.xhsimg.com` / `.hdslb.com` 等已知 CDN 下载，其他域名直接拒绝
- 所有数据写在本机目录，服务只监听 `127.0.0.1`
- OCR 用 macOS 系统框架跑，图片不出本机
- B 站解析器使用 Chrome 151 UA（B 站拒绝非浏览器 UA，无法匿名解析）；小红书保持自报 UA
...
同源移植版：Rust 进程内 sidecar（axum + tokio，端口 4318），API 契约与桌面完全一致，前端零改动。
替代浏览器扩展的入口是系统分享 —— 在小红书 App / 浏览器里「分享 → 收藏」，正文与链接自动入库。

- **导入**：SEND intent（text/plain）→ Kotlin bridge → 前端注册轮询 → 匿名解析 / 共享文本导入
- **本地能力**：规则分类 ✓；OCR（ML Kit）、语义模型 WASM、AI 分类为后续 Phase
- **数据位置**：/data/data/com.patrick.shoucang/（notes.json + media/）
- **构建**：全手动流程（绕过 tauri CLI 的 Windows symlink 与 android-studio-script bug），命令见 CLAUDE.md → Android 移植
