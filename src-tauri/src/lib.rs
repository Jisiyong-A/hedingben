//! 收藏 —— Tauri 桌面壳（Windows 主目标；Android 为内嵌 sidecar 变体）。
//! 桌面：spawn node sidecar (local-api.mjs, 端口 4318) + watchdog。
//! Android：无 node，同进程内嵌 Rust sidecar（server 模块），契约一致。
...
struct LocalApiState(Arc<Mutex<Option<Child>>>);
    ...
    #[cfg(desktop)]
    {
        let app = tauri::Builder::default()
            .setup(|app| {
                ...
                match spawn_local_api(app.handle()) {
                    Ok(mut child) => {
                        ...
                        start_watchdog(app_handle, child_arc);
                    }
                    Err(err) => {
                        eprintln!("{err}");
