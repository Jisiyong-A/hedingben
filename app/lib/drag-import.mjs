...
const SUPPORTED_DRAG_TYPES = new Set([
  'application/x-shoucang-note',
  'application/x-shoucang-card',
  'text/plain',
  'text/uri-list',
  'text/x-moz-url',
]);
...
export function parseDraggedCardInput(value) {
  ...
  try {
    ...
    const id = markerIndex >= 0
      ? payload?.id
      : sourceUrl.pathname.match(/^\/(?:explore|search_result|discovery\/item)\/([0-9a-f]{20,26})(?:\/|$)/i)?.[1];
    if (!/^[0-9a-f]{20,26}$/i.test(id || '')) return null;
